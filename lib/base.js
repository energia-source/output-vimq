(function (window) {
	...
	class Line {
		...
		static style(color) {
			...
			return properties.replaceAll('$s', VIMQ.stroke()).replaceAll('$c', color);
		}
		static coordinate(type, y, t, percentage) {
			let x = VIMQ.conversion(percentage);
	...
	class Render {
		...
		getFinal() {
			...
			this.elements.final.setAttribute('style', Line.style(color));
		...
		getText() {
			...
			this.elements.text.setAttribute('style', Info.style('#f00f00', 32));
		...
		setFinal(percentage) {
			let final = this.getFinal(),
				...
				t = VIMQ.height() - this.constructor.margin();

			coordinate.final = Line.coordinate(final, y, t, percentage);

			let position = t + Info.space() + 5,
	...
	class Rectangle {
		...
		getMin() {
			...
			this.elements.min.setAttribute('style', Line.style(color));
		...
		setMin(percentage) {
			let min = this.getMin(),
				...
				t = Xpower === this.getInfo().getType() ? y + this.constructor.calculate() : y - this.constructor.calculate();

			coordinate.min = Line.coordinate(min, y, t, percentage);
			this.render();
		...
		getFigure() {
			...
			let y = this.getInfo().getPosition(),
				t = Xpower === this.getInfo().getType() ? y + Info.space() + VIMQ.stroke() : y - Info.space() - this.constructor.height() - VIMQ.stroke(),
			...
			this.elements.figure.setAttribute('style', Line.style(color));
		...
		getMax() {
			...
			this.elements.max.setAttribute('style', Line.style(color));
		...
		setMax(percentage) {
			...
			coordinate.max = Line.coordinate(max, y, t, percentage);
			this.render();
		...
		render() {
			...
			this.elements.figure.setAttribute('x', coordinate.min);
			this.elements.figure.setAttribute('width', width);
		...
		static calculate() {
			return VIMQ.height() - 2 * Info.margin() + Info.space();
		}
	}
	...
	class Description {
		...
		getText() {
			...
			let type = Energy === this.getInfo().getType() ? Xpower : Energy;
			this.elements.element = document.createElementNS('http://www.w3.org/2000/svg', 'text');
			this.elements.element.setAttribute('style', Info.style('#000000', 16, 'left'));
		...
		setText(text) {
			let element = this.getText(),
				...
				y = Energy === this.getInfo().getType() ? position : position + 0.5 * Info.space(),
	...
	class Info {
		...
		constructor(vimq, type) {
			...
			this.type = type;
			this.elements = {};
			this.elements.rectangle = new Rectangle(this);
			this.elements.description = new Description(this);
		...
		getPosition() {
			return Xpower === this.getType() ? this.constructor.margin() - this.constructor.space() : VIMQ.height() - this.constructor.margin() + this.constructor.space();
		}
		getGroup() {
			if (this.elements.hasOwnProperty('group')) return this.elements.group;
		...
		setMin(percentage) {
			let min = this.getMin(),
				...
				space = 2 * VIMQ.stroke(),
				y = Xpower === this.getType() ? position - space : position + space + this.constructor.space();
		...
		getLine() {
			...
			this.elements.line.setAttribute('style', Line.style('#d12d2a'));
			return this.elements.line;
		}
		setLine(percentage) {
			let x = VIMQ.conversion(percentage).toString(),
				dimensions = this.constructor.space() + Rectangle.height(),
				y = this.getPosition(),
				coordinate = Xpower === this.getType() ? y + dimensions + VIMQ.stroke() : y - dimensions - VIMQ.stroke();
	...
	class VIMQ {
		...
		constructor() {
			this.elements = {};
			this.elements.render = new Render(this);
			this.elements.energy = new Info(this, Energy);
			this.elements.xpower = new Info(this, Xpower);
		}
		...
		getUp() {
			...
			this.elements.up.setAttribute('style', Line.style('#000000'));
			return this.elements.up;
		}
		getDown() {
			if (this.elements.hasOwnProperty('down')) return this.elements.down;
			let y = this.constructor.height() - Info.margin();
			...
			this.elements.down.setAttribute('style', Line.style('#000000'));
